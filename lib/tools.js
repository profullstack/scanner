...
function execWithLogging(command, options = {}, verbose = false) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn('sh', ['-c', command], {
      stdio: verbose ? ['pipe', 'pipe', 'pipe'] : ['pipe', 'pipe', 'pipe'],
      ...options
    });
...
export async function runNikto(target, options = {}) {
  ...
  try {
    // Use a more conservative timeout for Nikto (max 5 minutes per host)
    const niktoTimeout = Math.min(timeout, 300);
    let command = `nikto -host ${target} -Format ${format} -output ${outputFile} -timeout ${niktoTimeout}`;
    
    // Add Nikto-specific options to make it faster and less aggressive
    command += ' -maxtime 300'; // Maximum scan time in seconds
    command += ' -Tuning 1,2,3,4,5,6,7,8,9'; // Focus on common vulnerabilities
    ...
    const processTimeout = Math.min(timeout * 1000, 360000); // Max 6 minutes
...
export async function runZap(target, options = {}) {
  const { outputDir, timeout = 600, verbose = false, spider = true, auth = null, headers = {} } = options;
  ...
  try {
    ...
    if (auth) {
      ...
      if (auth.type === 'form' && auth.loginUrl && auth.loginData) {
        command += ` --auth-form --auth-url "${auth.loginUrl}" --auth-data "${auth.loginData}"`;
      }
      if (auth.sessionCookie) {
        command += ` --auth-cookie "${auth.sessionCookie}"`;
      }
    }
